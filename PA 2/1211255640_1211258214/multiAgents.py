...
class ReflexAgent(Agent):
    ...
    def evaluationFunction(self, currentGameState, action):
        ...
        # 2. Nearest active and scared Ghost
        ...
        if len(scaredghostdistances) >0:
            minscaredghostdist = min(scaredghostdistances)
            score = score+ (100.0/minscaredghostdist)
...
def betterEvaluationFunction(currentGameState):
    #### DESCRIPTION ####
    # To improve the Evaluation function, we considered:
    # 1. Nearest Food Pellets
    # 2. Closest Active Ghost
    # 3. Closest Scared Ghost
    #
    # 1. Food Pellets: First we, calculated the Manhattan distance from the agents to
    # all the food pellets. Then found the minimum of all the distances, which is the distance to closest food pellet.
    # Each food pellet is given a weightage of 10. Now we multiply the weight of food to the inverse of the closest distance.
    # This value is now added to the current score.
    #
    # 2. Ghosts: We get the total number of ghosts value. For each ghost, we check if it is a scared ghost or not.
    # We now find the distance from the agent to the ghost(active and scared stored in two different lists)
    # We now find the closest scared and active ghost using minimum fuction to their respective distance list.
    # If the ghost is scared ghost, it is given weightage of 100 and this weight is mutiplied with inverse of minimum distance.
    # This value is now added to score.
    #If the ghost is active ghost, weight 10 is multipled with inverse of minimum distance to active ghost
    # and this value is reduced from the score.
    #
    ##########
