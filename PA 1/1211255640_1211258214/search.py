...
def depthFirstSearch(problem):
    ...
    while fringe.isEmpty()==False:  # while loop to check while the stack is not empty
        ...
        if currentState in explored:    # adding current state to explored and marking it as visited
            continue
        else:
...
def breadthFirstSearch(problem):
    ...
    # just changing the util.Stack() to util.Queue() passed the test cases.
    # Reference:
...
def uniformCostSearch(problem):
    ...
    while fringe.isEmpty()==False:
        currentNode = fringe.pop()  # pop the node
        currentState = currentNode.state
        currentCost = currentNode.cost # get the cost of the current node
        ...
        for successor in problem.getSuccessors(currentState):
            if successor[0] not in explored:
                successorNode = node(currentNode,successor[0],successor[1],currentCost+successor[2])    # total Cost has to be calculated for 
...
def aStarSearch(problem, heuristic=nullHeuristic):
    ...
    fringe = util.PriorityQueue()
    fringe.push(root,root.fcost)    # pusing root node and f(n) of root node
