...
class CornersProblem(search.SearchProblem):
    ...
    def isGoalState(self, state):
        ...
        # Final goal is to visit all corners, so, we check if corvers visited is (1,1,1,1) or not. 
    ...
    def getSuccessors(self, state):
        ...
        for action in [Directions.NORTH, Directions.SOUTH, Directions.EAST, Directions.WEST]:
            ...
            if hitsWall==False: # does not hit wall
                ...
                # state[1] is a tuple, and thus immutable that is why we convert it to list to make changes
...
def cornersHeuristic(state, problem):
    ...
    for i in range(4):
        if visitedCorners[i]==0:
            unvisitedCorners.append(corners[i]) # creates a list of currently unvisited corners
...
class AnyFoodSearchProblem(PositionSearchProblem):
    ...
    def isGoalState(self, state):
        ...
        # util.raiseNotDefined()
        # The goal is to check if food is there or not in the state
